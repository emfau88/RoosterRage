...
export function preloadGameAssets(scene) {
  ...
  Object.entries(ROOSTER_ASSET_URLS).forEach(([roosterId, assets]) => {
    scene.load.spritesheet(`rooster-${roosterId}-walk`, assets.walk, { frameWidth: 256, frameHeight: 256 });
    scene.load.spritesheet(`rooster-${roosterId}-idle`, assets.idle, { frameWidth: 256, frameHeight: 256 });
    if (assets.legacyWalk) {
      scene.load.spritesheet(`rooster-${roosterId}-walk-legacy`, assets.legacyWalk, { frameWidth: 256, frameHeight: 256 });
    }
  });
