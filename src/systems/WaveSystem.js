export class WaveSystem {
  ...
  makeSlime(multiplier = 1) {
    return { type: 'slime', hp: Math.round(44 * multiplier), speed: 74, damage: 8, xp: 8, texture: 'enemy-slime', scale: 0.24, radius: 28, bodyOffsetX: 100, bodyOffsetY: 118, hpBarWidth: 42, hpBarYOffset: 32 };
  }

  makeRunner(multiplier = 1) {
    return { type: 'runner', hp: Math.round(48 * multiplier), speed: 112, damage: 9, xp: 9, texture: 'enemy-runner', scale: 0.25, radius: 24, bodyOffsetX: 104, bodyOffsetY: 122, hpBarWidth: 40, hpBarYOffset: 30 };
  }

  makeBrute(multiplier = 1) {
    return { type: 'brute', hp: Math.round(95 * multiplier), speed: 74, damage: 13, xp: 13, texture: 'enemy-brute', scale: 0.27, radius: 34, bodyOffsetX: 94, bodyOffsetY: 108, hpBarWidth: 54, hpBarYOffset: 42 };
  }

  makeSpitter(multiplier = 1) {
    return { ...this.makeSlime(0.9 * multiplier), type: 'spitter', speed: 58, damage: 7, xp: 12, texture: 'enemy-spitter', scale: 0.26, radius: 27, bodyOffsetX: 101, bodyOffsetY: 102, ability: { kind: 'shoot', cooldown: 2200, speed: 230, damage: 7, texture: 'enemy-shot', radius: 7, color: 0x7cff67, trailColor: 0x4dea7e } };
  }

  makeFanSpitter(multiplier = 1) {
    return { ...this.makeSlime(1.05 * multiplier), type: 'fan-spitter', speed: 52, damage: 8, xp: 14, texture: 'enemy-fan-spitter', scale: 0.29, radius: 31, bodyOffsetX: 97, bodyOffsetY: 100, hpBarWidth: 48, hpBarYOffset: 36, ability: { kind: 'fan', cooldown: 3100, speed: 210, damage: 6, texture: 'enemy-shot', radius: 7, count: 3, spread: 0.75, color: 0x6bd8ff, trailColor: 0x51a8ff } };
  }

  makeBomber(multiplier = 1) {
    return { ...this.makeRunner(0.95 * multiplier), type: 'bomber', speed: 92, damage: 12, xp: 13, texture: 'enemy-bomber', scale: 0.28, radius: 29, bodyOffsetX: 99, bodyOffsetY: 100, hpBarWidth: 46, hpBarYOffset: 36, explodeOnDeath: true, explosionRadius: 86, explosionDamage: 18 };
  }

  makeEliteRunner() {
    return { ...this.makeRunner(3.1), type: 'elite-runner', elite: true, eliteTint: false, speed: 128, damage: 13, xp: 40, texture: 'enemy-elite-runner', scale: 0.34, radius: 29, bodyOffsetX: 99, bodyOffsetY: 100, hpBarWidth: 62, hpBarYOffset: 42 };
  }

  makeEliteBrute() {
    return { ...this.makeBrute(3.2), type: 'elite-brute', elite: true, eliteTint: false, speed: 68, damage: 18, xp: 48, texture: 'enemy-elite-brute', scale: 0.39, radius: 41, bodyOffsetX: 87, bodyOffsetY: 91, hpBarWidth: 74, hpBarYOffset: 56 };
  }

  makeEliteSpitter() {
    return { ...this.makeSpitter(2.7), type: 'elite-spitter', elite: true, eliteTint: false, speed: 50, xp: 44, texture: 'enemy-elite-spitter', scale: 0.34, radius: 36, bodyOffsetX: 92, bodyOffsetY: 93, hpBarWidth: 68, hpBarYOffset: 48, ability: { kind: 'fan', cooldown: 2400, speed: 230, damage: 7, texture: 'enemy-shot', radius: 7, count: 3, spread: 0.7, color: 0xb86cff, trailColor: 0x7a5cff } };
  }

  makeBoss() {
    return { ...this.makeBrute(50), type: 'boss', boss: true, elite: true, eliteTint: false, speed: 58, damage: 24, xp: 120, texture: 'enemy-boss', scale: 0.58, radius: 58, bodyOffsetX: 70, bodyOffsetY: 72, hpBarWidth: 130, hpBarYOffset: 88, ability: { kind: 'fan', cooldown: 1900, speed: 280, damage: 12, texture: 'enemy-shot', radius: 8, count: 5, spread: 1.15, color: 0xff7a33, trailColor: 0xff3828 } };
  }
}
