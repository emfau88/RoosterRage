"""Bake the exact live Ace rig poses into portable direction sheets and QA GIFs."""
...
QA = ROOT / "docs/qa/ace-four-direction"
RUNTIME = ROOT / "src/assets/characters/ace-next"
SIZE = 256
SCALE = 2
...
def render(pose, images):
    ...
    # Remove sub-visible resampling/chroma residue before GPU texture filtering can
    # turn it into one-pixel lines between packed runtime frames.
...
def preview(frame, label):
    ...
    draw.text((160, 280), label, font=font(12), fill="#cbb46d", anchor="mm")
    return canvas
...
def main():
    ...
    neutral_frames = {}
    runtime_frames = {}
    manifest = {"version": 1, "frameWidth": SIZE, "frameHeight": SIZE, "columns": 8,
                "eastDerivedFrom": "west-mirrored", "clips": {}}
    for clip in data["clips"]:
        ...
        name = f"ace-{clip['direction']}-{clip['mode']}"
        sheet.save(EXPORT / f"{name}.png", optimize=True)
        sheet.save(EXPORT / f"{name}.webp", lossless=True, method=6)
        gif_frames = [preview(frame, f"ACE / {clip['direction'].upper()} / {clip['mode'].upper()}") for frame in frames]
        gif_duration = round(clip["duration"] / len(frames) / 10) * 10
        gif_frames[0].save(QA / f"{name}.gif", save_all=True, append_images=gif_frames[1:],
                           duration=gif_duration, loop=0, optimize=False, disposal=2)
        manifest["clips"][f"{clip['direction']}-{clip['mode']}"] = {
            "file": f"{name}.webp", "frameCount": len(frames), "durationMs": clip["duration"],
            "frameRate": len(frames) * 1000 / clip["duration"], "repeat": -1,
        }
        if clip["mode"] == "idle":
        ...
        reports.append({
            "direction": clip["direction"], "mode": clip["mode"], "frames": len(frames),
            "minimumMarginPx": minimum_margin, "widthRangePx": [min(widths), max(widths)],
    ...
    contact.save(QA / "ace-four-directions-neutral.png")
    ...
    (QA / "asset-check.json").write_text(json.dumps({"clips": reports, "frameSize": SIZE,
        "directions": ["south", "west", "north", "east"], "eastMirrorsWest": True,
        "sourcePartIdentity": "Same immutable direction parts in every frame"}, indent=2) + "\n", encoding="utf-8")
