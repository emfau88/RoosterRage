"""Bake the live Bummbert rig poses into runtime sheets and review artifacts."""
...
SOURCE = ROOT / "art-source/characters/artillery-production-v1"
PARTS = ROOT / "src/assets/characters/artillery-four-direction"
EXPORT = SOURCE / "exports/four-direction"
QA = ROOT / "docs/qa/artillery-four-direction"
RUNTIME = ROOT / "src/assets/characters/artillery-next"
SIZE = 256
SCALE = 2
...
def main():
    ...
    for clip in data["clips"]:
        ...
        name = f"artillery-{clip['direction']}-{clip['mode']}"
        sheet.save(EXPORT / f"{name}.png", optimize=True)
        sheet.save(EXPORT / f"{name}.webp", lossless=True, method=6)
        gif_frames = [preview(frame, f"BUMMBERT / {clip['direction'].upper()} / {clip['mode'].upper()}") for frame in frames]
    ...
    contact.save(QA / "artillery-four-directions-neutral.png")
    ...
    for mode, spec in runtime_specs.items():
        ...
        filename = f"rooster-artillery-next-{mode}.webp"
